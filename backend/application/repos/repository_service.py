...
def get_repository_summary(session: Session, repository_id: UUID) -> dict:
    ...
    file_count = (
        session.query(func.count(FileModel.id))
        .filter_by(repository_id=repository_id)
        .scalar()
        or 0
    )
    function_count = (
        session.query(func.count(FunctionModel.id))
        .join(FileModel)
    ...
    chunk_count = (
        session.query(func.count(ChunkModel.id))
        .filter_by(repository_id=repository_id)
        .scalar()
        or 0
    )
...
def _remove_upload_artifacts(root_path: str | None) -> bool:
    """Remove extracted ZIP tree under data/uploads when applicable."""
    if not root_path:
        return False
    ...
    try:
        ...
        logger.debug(
            "repository_service: root_path %s is outside uploads; skipping disk tree removal",
            root,
        )
        return False
...
def purge_repository_artifacts(repo: RepositoryModel) -> dict[str, bool]:
    ...
    return {
        "faiss_index": _remove_faiss_index(repository_id),
        "upload_tree": _remove_upload_artifacts(repo.root_path),
    }

def delete_repository(session: Session, repository_id: UUID) -> dict | None:
    """
    Delete repository and all related data: Postgres rows (cascade), FAISS index,
    and extracted upload files when stored under data/uploads.
    """
    repo = session.get(RepositoryModel, repository_id)
    if repo is None:
        return None
    ...
    session.commit()
    logger.info(
        "repository_service: deleted repository id=%s name=%r artifacts=%s",
        repository_id,
        repo.name,
        artifacts,
    )
