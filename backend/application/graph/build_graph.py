...
def build_hierarchy(session: Session, repository_id: UUID) -> dict:
    ...
    files = (
        ...
        .options(
            joinedload(FileModel.classes),
            joinedload(FileModel.functions).joinedload(FunctionModel.class_),
        )
        .all()
    )
    ...
    def ensure_folder(parts: list[str]) -> str:
        ...
        folder_nodes[path_key] = {
            "id": fid,
            "type": NODE_FOLDER,
            "name": parts[-1] if parts else repo.name,
            "parent_id": parent_id,
            "path": path_key,
            "children": [],
        }
...
def build_architecture_graph(session: Session, repository_id: UUID) -> dict:
    ...
    calls = (
        ...
        .join(FunctionModel, FunctionCallModel.caller_function_id == FunctionModel.id)
        .join(FileModel, FunctionModel.file_id == FileModel.id)
        .filter(FileModel.repository_id == repository_id)
        .options(
            joinedload(FunctionCallModel.caller).joinedload(FunctionModel.file),
            joinedload(FunctionCallModel.callee).joinedload(FunctionModel.file),
        )
        .all()
    )
    for call in calls:
    ...
    logger.info(
        "build_graph: repository=%s nodes=%d edges=%d",
        repository_id,
        len(nodes),
        len(edges),
    )
