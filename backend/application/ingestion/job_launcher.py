...
def launch_ingestion_worker(
    ...
    *,
    ...
    """Start ingestion in a detached process (survives uvicorn --reload)."""
    ...
    try:
        ...
        logger.info(
            "job_launcher: started worker pid=%s job=%s",
            proc.pid,
            job.id,
        )
        return proc
...
def resume_recoverable_jobs(*, max_upload_bytes: int) -> int:
    """Restart jobs left running/failed after a server reload or crash."""
...
def maybe_resume_queued_job(*, max_upload_bytes: int) -> bool:
    """Start the next recoverable job after the active worker finishes."""
