"""Verify ingestion worker launch and recovery of interrupted jobs.

Covers ``application.ingestion.job_launcher`` and
``application.repos.processing_tracker`` for PID recording and restart logic.
"""
...
def test_resume_recoverable_jobs_restarts_failed_interrupt(tmp_path, monkeypatch):
    ...
    with patch('application.ingestion.job_launcher.launch_ingestion_worker', return_value=mock_proc) as launch:
