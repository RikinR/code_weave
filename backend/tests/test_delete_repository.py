"""Verify repository deletion removes FAISS indexes and upload artifacts.

Covers ``application.repos.repository_service`` helpers for FAISS index removal,
upload directory cleanup, and combined ``purge_repository_artifacts`` behavior.
"""
...
def test_remove_faiss_index(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr('application.repos.repository_service.faiss_index_path_for_repository', lambda rid: index_dir / f'{rid}.index')
...
def test_remove_upload_artifacts_deletes_job_folder(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr('application.repos.repository_service.UPLOADS_DIR', uploads)
...
def test_remove_upload_artifacts_skips_outside_uploads(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr('application.repos.repository_service.UPLOADS_DIR', tmp_path / 'uploads')
