"""Verify repository-relative path resolution for ingestion and graph building.

Covers ``application.ingestion.path_utils``, ``application.graph.build_graph``
folder helpers, and legacy absolute upload paths on ``RepositoryModel``.
"""
...
def test_repo_relative_path_legacy_absolute():
    repo = RepositoryModel(name='rag_prac', root_path='/data/uploads/uuid/rag_prac')
