"""Groq chat completion client for RAG answer generation.

Wraps the Groq SDK with logging and error translation. Called synchronously or
as a stream from :mod:`application.retrieval.rag_service` and the chat API routes.
"""
...
def chat_completion_stream(messages: list[dict], model: str | None=None, *, temperature: float=0.2, max_tokens: int=1024):
    """Yield assistant text deltas from a streaming Groq chat completion."""
    if not messages:
        raise GroqError('messages must not be empty')
    model_name = model or GROQ_MODEL
    try:
        stream = _client().chat.completions.create(model=model_name, messages=messages, temperature=temperature, max_tokens=max_tokens, stream=True)
        for chunk in stream:
