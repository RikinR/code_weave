"""Verify processing job lifecycle, persistence, and restart-safe status handling.

Covers ``application.repos.processing_tracker.ProcessingTracker`` stage
transitions, JSON persistence, and recovery when workers are interrupted.
"""
...
def test_job_persisted_and_reloaded(tmp_path, monkeypatch):
    monkeypatch.setattr('application.repos.processing_tracker.JOBS_DIR', tmp_path)
    tracker = ProcessingTracker()
...
def test_running_job_with_dead_worker_pid_is_stale(tmp_path, monkeypatch):
    ...
    tracker.set_worker_pid(job, 999999999)
...
def test_stale_running_job_interrupted_on_poll(tmp_path, monkeypatch):
    monkeypatch.setattr('application.repos.processing_tracker.JOBS_DIR', tmp_path)
    monkeypatch.setattr('application.repos.processing_tracker.STALE_JOB_SECONDS', 30)
    tracker = ProcessingTracker()
...
def test_orphan_zip_recovered_on_get_job(tmp_path, monkeypatch):
    ...
    (uploads / f'{job_id}.meta.json').write_text('{"repository_name": "my-repo"}', encoding='utf-8')
    tracker = ProcessingTracker()
