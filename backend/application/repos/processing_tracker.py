"""In-memory and on-disk tracking of ZIP ingestion pipeline jobs.

Persists progress to ``data/jobs/{job_id}.json``, exposes SSE updates via
:func:`sse_event_stream`, and coordinates with upload artifacts under
``data/uploads/``. Used by upload/progress API routes and the ingestion worker.
"""
...
from application.repos.job_file_lock import job_file_lock
...
class StageStatus(str, Enum):
    ...
    COMPLETED = 'completed'
    FAILED = 'failed'
PIPELINE_STAGES: tuple[str, ...] = ('zip_extraction', 'file_scanning', 'language_detection', 'parsing', 'graph_persist', 'embedding_generation', 'vector_storage', 'architecture_graph', 'indexing_complete')
...
class StageState:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {'key': self.key, 'label': self.label, 'status': self.status.value, 'progress': self.progress, 'logs': list(self.logs)}
_STAGE_LABELS: dict[str, str] = {'zip_extraction': 'ZIP extraction', 'file_scanning': 'File scanning', 'language_detection': 'Language detection', 'parsing': 'Parse & read files', 'graph_persist': 'Persist graph to database', 'embedding_generation': 'Embedding generation', 'vector_storage': 'Vector storage', 'architecture_graph': 'Architecture graph', 'indexing_complete': 'Indexing complete'}

@dataclass
class IngestionJob:
    """Mutable job state with staged pipeline progress and subscriber notifications."""
    ...
    _listeners: list[Callable[[dict[str, Any]], None]] = field(default_factory=list, repr=False)
    _lock: threading.Lock = field(default_factory=threading.Lock, repr=False)

    def __post_init__(self) -> None:
        if not self.stages:
            self.stages = {key: StageState(key=key, label=_STAGE_LABELS.get(key, key)) for key in PIPELINE_STAGES}

    def snapshot(self) -> dict[str, Any]:
        with self._lock:
            return {'job_id': self.id, 'repository_name': self.repository_name, 'status': self.status, 'repository_id': self.repository_id, 'worker_pid': self.worker_pid, 'error': self.error, 'created_at': self.created_at, 'updated_at': self.last_activity_at, 'stages': [self.stages[k].to_dict() for k in PIPELINE_STAGES]}

    @classmethod
    def from_snapshot(cls, data: dict[str, Any]) -> IngestionJob:
        job = cls(id=str(data['job_id']), repository_name=str(data['repository_name']), status=str(data.get('status', 'pending')), repository_id=data.get('repository_id'), worker_pid=int(data['worker_pid']) if data.get('worker_pid') is not None else None, error=data.get('error'), created_at=float(data.get('created_at', time.time())), last_activity_at=float(data.get('updated_at', data.get('created_at', time.time()))))
...
class ProcessingTracker:
    """Registry of ingestion jobs with disk persistence, staleness detection, and SSE hooks."""
    ...
    def save_upload_meta(self, job_id: str, repository_name: str) -> None:
        ...
        try:
            path.write_text(json.dumps({'repository_name': repository_name}), encoding='utf-8')
        except OSError:
    ...
    def _recover_orphan_job(self, job_id: str) -> IngestionJob | None:
        ...
        logger.warning('processing_tracker: recovered orphan job %s (zip=%s extract=%s)', job_id, zip_path.is_file(), extract_dir.is_dir())
        return job
    ...
    def create_job(self, repository_name: str) -> IngestionJob:
        """Create a new pending job, persist metadata, and return the in-memory instance."""
    ...
    def get_job(self, job_id: str) -> IngestionJob | None:
        """Load a job from disk cache, memory, or recover from orphan upload artifacts."""
        path = self._job_path(job_id)
        if path.is_file():
...
async def sse_event_stream(job_id: str):
    """Async generator of SSE snapshot/update events for pipeline UI subscribers."""
